...
class Review extends \Opencart\System\Engine\Controller {
	...
	public function write(): void {
		...
		if (isset($this->request->post['value_for_money'])) {
			$value_for_money = $this->request->post['value_for_money'];
		} else {
			$value_for_money = 0; // Default value
		}
		...
		if ($this->request->post['value_for_money'] < 1 || $this->request->post['value_for_money'] > 5) {
			$json['error']['value_for_money']  = $this->language->get('error_rating');
		}
	...
	public function getList(): string {
		...
		foreach ($results as $result) {
			$data['reviews'][] = [
				...
				'value_for_money'     => (int)$result['value_for_money'],
